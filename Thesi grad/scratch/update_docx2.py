...
# NOTE: The backup was already created and paragraphs were already successfully updated in the previous run.
# However, the previous run crashed before saving, so no changes were saved! We need to do it all again safely.
...
for tbl in doc.tables:
    for idx, row in enumerate(tbl.rows):
        if len(row.cells) == 6 and 'Fe/N-CA' in row.cells[0].text:
