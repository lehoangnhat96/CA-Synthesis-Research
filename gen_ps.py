...
# Add the citation replacements (longest first to avoid partial matches, though exact match on brackets helps)
# Need to escape single quotes inside strings if any, but our strings only contain brackets and numbers/commas/dashes.
for old_s in sorted(replacements.keys(), key=len, reverse=True):
    new_s = replacements[old_s]
    ps_script += f"Replace-Text '{old_s}' '{new_s}'\n"

# Finally, convert << and >> to [ and ]
ps_script += "Replace-Text '<<' '['\n"
ps_script += "Replace-Text '>>' ']'\n"
...
with open('run_word.ps1', 'w', encoding='utf-8') as f:
    f.write(ps_script)
