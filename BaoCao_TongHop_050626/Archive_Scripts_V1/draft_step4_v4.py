...
draft = f"""# CHI TIẾT BẢN NHÁP CHƯƠNG 3 (BƯỚC 4 - Đã chỉnh sửa V4)

> Đã lược bỏ phần phân tích Paracetamol. Ký hiệu Fe²⁺/Fe³⁺ đã format chuẩn.
> **Khôi phục Nafion:** Đã bổ sung lại vai trò của màng Nafion kết hợp với Chitosan (thành hệ Composite Binder) trong phân tích kim loại nặng theo đúng nguyên lý.

---

## 3.2. Dự kiến Kết quả Đặc trưng Hình thái và Hóa lý
...
## 3.3. Dự kiến Tính chất Điện hóa Cơ bản (CV, EIS)
...
## 3.4. Dự kiến Thẩm định Phân tích Vết Kim loại nặng

*Bảng này sẽ thay thế cho Bảng 3.2 cũ trong DCLV*
...
**Biện luận cơ chế đa chức năng của Composite Binder (Chitosan + Nafion):**
LOD siêu nhạy (< 0.1 ppb đối với chì) đạt được nhờ sự hiệp đồng của hệ "mỏ neo kép". Cấu trúc xốp phân cấp của carbon aerogel rút ngắn quãng đường khuếch tán, trong khi các tâm Fe-N₄ phối trí trực tiếp với ion kim loại. Đặc biệt, màng kết dính **Composite (Chitosan + 0.5% Nafion)** đóng vai trò chủ chốt: Chitosan đóng vai trò "nam châm" làm giàu hóa học, tạo phức chelate mạnh mẽ để bẫy các cation Pb²⁺/Cd²⁺/Zn²⁺, trong khi khung perfluorocarbon kỵ nước của màng **Nafion** giúp cản trở các phân tử hữu cơ lớn bám rễ, ngăn ngừa triệt để hiện tượng bám bẩn (fouling) khi phân tích mẫu thực tế [45]. 
...
with open(draft_path, 'w', encoding='utf-8') as f:
    f.write(draft)
print("Draft for Chapter 3 V4 generated.")
